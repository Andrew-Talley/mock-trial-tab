...
class Round(graphene.ObjectType):
  ...
  def resolve_matchups(parent, info):
    matchups = SQLRound.get_matchups_for_round(parent.tournament_id, parent.round_num)
    return [Matchup(id=matchup['id'], pl=Team(num=matchup['pl']), defense=Team(num=matchup['def'])) for matchup in matchups]