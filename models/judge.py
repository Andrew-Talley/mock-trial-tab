...
class Judge:
  ...
  def add_judge(tournament_id: int, name: str):
    cursor = db.cursor()
    cursor.execute(f"INSERT INTO {judge_table} (tournament_id, name) VALUES (%s, %s)", (tournament_id, name))
  ...
  def get_judge(tournament_id: int, id: int):
    cursor = db.cursor()
    cursor.execute(f"SELECT tournament_id, id, name FROM {judge_table} WHERE tournament_id = %s AND id = %s", (tournament_id, id))
  ...
  def add_conflict(tournament_id: int, id: int, school: str):
    cursor = db.cursor()
    cursor.execute(f"INSERT INTO {conflict_table} (tournament_id, judge_id, school_name) VALUES (%s, %s, %s)", (tournament_id, id, school))
  ...
  def get_conflicts(tournament_id: int, id: int):
    cursor = db.cursor()
    cursor.execute(f"SELECT school_name FROM {conflict_table} WHERE judge_id = %s", (id, ))
