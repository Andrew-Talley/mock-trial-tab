...
class School:
  ...
  def add_school(tournament_id: int, name: str):
    cursor = db.cursor()
    cursor.execute(f"INSERT INTO {school_table} (tournament_id, name) VALUES (%s, %s)", (tournament_id, name))
  ...
  def get_teams_for_school(tournament_id: int, name: str):
    cursor = db.cursor()
    cursor.execute(f"SELECT team_num, name FROM {team_table} WHERE tournament_id = %s AND school_name = %s", (tournament_id, name))
