...
class Team:
  ...
  def get_team(tournament_id, num):
    cursor = db.cursor()
    cursor.execute(f"SELECT name, school_name FROM {team_table} WHERE tournament_id = %s AND team_num = %s", (tournament_id, num))
  ...
  def get_students(tournament_id, num):
    cursor = db.cursor()
    cursor.execute(f"SELECT id, student_name FROM {student_table} WHERE tournament_id = %s AND team_num = %s", (tournament_id, num))
